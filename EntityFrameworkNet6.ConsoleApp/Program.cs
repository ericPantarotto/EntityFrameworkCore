...
using EntityFrameworkNet6.ConsoleApp;
using Microsoft.EntityFrameworkCore;
using Newtonsoft.Json;



FootBallLeagueDbContext context = new();
  ...
  async Task AddTeamsWithLeague(League league)
{
  ...
  async Task AddTeamLeagueTogether()
{
  ...
  async Task SimpleQueryJson()
{
    ...
    var myEntity = context.Leagues.Select(l => new { l.Id, l.Name, TestResultOnName = !string.IsNullOrWhiteSpace(l.Name) });
  ...
  async Task QueryFiltersInput()
{
  ...
  async Task QueryFiltersContains()
{
    Console.Write("Enter League Name (or part of): ");
    var leagueName = Console.ReadLine();
  ...
  async Task SimpleUpdateLeagueRecord()
{
  ...
  async Task<League> GetLeagueRecord(int id)
{
    return await context.Leagues.FindAsync(id);
}
  async Task<Team> GetTeamRecord(int id)
{
  ...
  async Task SimpleUpdateTeamRecord()
{
  ...
  async Task SimpleUpdateTeamRecordTestUpdateDate()
{
    var team = new Team { Id = 20, Name = "Date = Eric Carlier - Sample Team ", LeagueId = 1010 };
    context.Teams.Update(team);
    await context.SaveChangesAsync();
}
  async Task SimpleUpdateTeamRecordWithAuditContext()
{
  ...
  async Task TrackingVsNoTracking()
{
  ...
  async Task AddNewTeamWithLeague()
{
  ...
  async Task ExecuteNonQueryCommand()
{
  ...
  async Task TestingAuditTableOnCoach()
{
...
async Task TestModelQueryRelatedRecords()
{
    //get many related records: Leagues => Teams
